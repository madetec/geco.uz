(function() {
	...
	var viewEl = document.querySelector('.view'),
		...
		items = [].slice.call(gridEl.querySelectorAll('.product3')),
		basket;
	...
	$(".action--close").on("click", function () {
                $(".action--remove").click();
                $("#firstCheckbox").click();
                $(".compare-basket").removeClass("compare-basket--active");
            });
	...
	CompareBasket.prototype._createItemPreview = function(item) {
		...
		removeCtrl.innerHTML = '<i class="material-icons"></i><span class="action__text action__text--invisible">Remove product</span>';
		removeCtrl.addEventListener('click', function() {
			self.remove(item);
		});

		
		var productImageEl = item.querySelector('div.product__image').cloneNode(true);
	...
	CompareBasket.prototype._compareItems = function() {
		...
		for(var i = 0; i < this.totalItems; ++i) {
			...
			this.compareWrapper.insertBefore(compareItemWrapper, this.compareWrapper.childNodes[i]);
		}
	...
	function initEvents() {
		...
		items.forEach(function(item) {
			...
			var firstCheckbox =item.querySelector('.action--compare-add > #firstCheckbox');
